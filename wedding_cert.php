...
global $pdo;
$query = "SELECT *, DATE_FORMAT(baptismal_date, '%d/%m/%Y') AS date_component, TIME_FORMAT(baptismal_date, '%h:%i %p') AS time_component FROM binyag_request_certificate";
    ...
    <nav>
        <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
            <a class="nav-item nav-link active" id="nav-process-tab" data-toggle="tab" href="#nav-process"
                ...
                <span class="badge badge-primary rounded-circle p-2"><?php echo $total_baptismalrequest; ?></span>
                <?php endif; ?>
            </a>
...
<tbody>
    <?php foreach ($inventory as $item) { ?>
        <tr>
            ...
            <td class="text-center align-middle"><?php echo $item['baptismal_date']; ?></td>
            <td class="text-center align-middle"><?php echo $item['baptized_by']; ?></td>
            <td class="text-center align-middle"><?php echo $item['godfather']; ?></td>
            <td class="text-center align-middle"><?php echo $item['godmother']; ?></td>
            <td class="text-center align-middle">
    <button><a href="forms/baptism-cert.php"  data-id="<?php echo $item['id']; ?>">Generate</a></button>
</td>
</tr>
        </tr>
                    <!-- MODAL APPLICATION -->
...
fetch(`forms/baptism-cert.php?id=${ id }`).then(response => response.text()).then(data => {
