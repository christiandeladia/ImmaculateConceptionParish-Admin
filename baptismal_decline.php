...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if(isset($_POST['dataId'])){
        ...
        $result = mysqli_query($conn, "SELECT * FROM binyag WHERE id = $dataId");
        ...
        if ($result) {
            ...
            $updateQuery = "UPDATE binyag SET reason=?, remarks=?, status_id=? WHERE id=?";
            ...
            if ($updateResult) {
                ...
                VALUES (?, 'Baptismal', 'unread', ?, ?, 'Decline', ?, ?)";
        ...
        echo "<script> console.error('AJAX Error:', status, error);</script>";
    }
} else {
